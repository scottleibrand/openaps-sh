...
openaps alias add get-bg '! bash -c "( openaps monitor-cgm 2>/dev/null && grep -q glucose monitor/cgm-glucose.json && rsync -rtu monitor/cgm-glucose.json monitor/glucose.json ) || ( openaps monitor-share 2>/dev/null >/dev/null ) && rsync -rtu monitor/glucose.json glucose.json || ( find glucose.json -mmin -5 | egrep -q glucose && rsync -rtu glucose.json monitor/glucose.json && echo Copied glucose.json to monitor/glucose.json ) || ( openaps get-ns-glucose && grep -q glucose monitor/ns-glucose.json && mv monitor/ns-glucose.json monitor/glucose.json )"' || die "Can't add get-bg"
