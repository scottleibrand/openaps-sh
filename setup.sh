...
openaps alias add ns-meal-carbs '! bash -c "curl -m 30 -s \"$NIGHTSCOUT_HOST/api/v1/treatments.json?find\[carbs\]\[\$exists\]=true\" > settings/carbhistory.json; oref0-meal monitor/pumphistory-zoned.json settings/profile.json monitor/clock-zoned.json settings/carbhistory.json monitor/glucose.json settings/basal_profile.json > monitor/meal.json; exit 0"' || die "Can't add ns-meal-carbs"
