...
openaps alias add refresh-old-pumphistory '! bash -c "find monitor/ -mmin -30 | grep -q pumphistory-zoned || (openaps gather && echo Refreshed old pumphistory)"'
openaps alias add refresh-old-pumphistory-24h '! bash -c "find monitor/ -mmin -30 | grep -q pumphistory-24h-zoned || openaps report invoke settings/pumphistory-24h.json settings/pumphistory-24h-zoned.json"'
...
openaps alias add refresh-pumphistory-24h '! bash -c "find settings/ -mmin -15 | grep -q pumphistory-24h-zoned || openaps report invoke settings/pumphistory-24h.json settings/pumphistory-24h-zoned.json"'
...
openaps alias add pump-loop '! bash -c "until(echo Starting pump-loop: && openaps wait-for-silence && openaps refresh-old-pumphistory && openaps refresh-old-pumphistory-24h && openaps refresh-old-profile && openaps refresh-temp-and-enact && openaps refresh-pumphistory-and-enact && openaps refresh-profile && openaps refresh-pumphistory-24h && echo Completed pump-loop); do openaps wait-for-silence && openaps mmtune; done"' || die "Can't add pump-loop"
...
(crontab -l; crontab -l | grep -q ns-loop || echo "* * * * * cd $directory && ps aux | grep -v grep | grep -q 'openaps ns-loop' || ( date; openaps ns-loop ) | tee -a /var/log/openaps/ns-loop.log") | crontab -
