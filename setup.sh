...
openaps alias add refresh-old-pumphistory-24h '! bash -c "find settings/ -mmin -60 -size +100c | grep -q pumphistory-24h-zoned || (echo -n Old pumphistory-24h refresh && openaps report invoke settings/pumphistory-24h.json settings/pumphistory-24h-zoned.json 2>/dev/null >/dev/null && echo ed)"'
# if glucose.json is newer than temp_basal.json, or temp_basal.json is more than 5m old, refresh-temp-and-enact
openaps alias add refresh-temp-and-enact '! bash -c "if( (find monitor/ -newer monitor/temp_basal.json | grep -q glucose.json && echo glucose.json newer than temp_basal.json ) || (! find monitor/ -mmin -5 -size +5c | grep -q temp_basal && echo temp_basal.json more than 5m old)); then (echo -n Temp refresh && openaps report invoke monitor/temp_basal.json monitor/clock.json monitor/clock-zoned.json monitor/iob.json 2>/dev/null >/dev/null && echo ed && openaps enact); else echo temp_basal.json less than 5m old; fi"'
...
openaps alias add refresh-pumphistory-24h '! bash -c "find settings/ -mmin -10 -size +100c | grep -q pumphistory-24h-zoned && echo Pumphistory-24 less than 10m old || (echo -n pumphistory-24h refresh && openaps report invoke settings/pumphistory-24h.json settings/pumphistory-24h-zoned.json 2>/dev/null >/dev/null && echo ed)"'
