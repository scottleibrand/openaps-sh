...
openaps alias add refresh-old-pumphistory '! bash -c "find monitor/ -mmin -30 -size +100c | grep -q pumphistory-zoned || ( echo -n \"Old pumphisotry: \" && openaps gather && openaps enact ) "'
openaps alias add refresh-old-pumphistory-24h '! bash -c "find settings/ -mmin -30 -size +100c | grep -q pumphistory-24h-zoned || (echo -n Old pumphistory-24h refresh && openaps report invoke settings/pumphistory-24h.json settings/pumphistory-24h-zoned.json 2>/dev/null >/dev/null && echo ed)"'
...
openaps alias add refresh-old-profile '! bash -c "find settings/ -mmin -30 -size +5c | grep -q profile && echo Profile less than 30m old || (echo -n Old settings refresh && openaps get-settings 2>/dev/null >/dev/null && echo ed )"'
openaps alias add refresh-profile '! bash -c "find settings/ -mmin -15 -size +5c | grep -q settings.json && echo Profile less than 15m old || (echo -n Settings refresh && openaps get-settings 2>/dev/null >/dev/null && echo ed)"'
openaps alias add refresh-pumphistory-24h '! bash -c "find settings/ -mmin -15 -size +100c | grep -q pumphistory-24h-zoned && echo Pumphistory-24 less than 15m old || (echo -n pumphistory-24h refresh && openaps report invoke settings/pumphistory-24h.json settings/pumphistory-24h-zoned.json 2>/dev/null >/dev/null && echo ed)"'
