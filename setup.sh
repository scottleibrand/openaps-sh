...
openaps alias add refresh-old-pumphistory '! bash -c "find monitor/ -mmin -30 -size +100c | grep -q pumphistory-zoned || ( echo -n \"Old pumphistory: \" && openaps gather && openaps enact ) "'
...
openaps alias add refresh-temp-and-enact '! bash -c "if( (find monitor/ -newer monitor/temp_basal.json | grep -q glucose.json && echo glucose.json newer than temp_basal.json ) || (! find monitor/ -mmin -5 -size +5c | grep -q temp_basal && echo temp_basal.json more than 5m old)); then (echo -n Temp refresh && openaps report invoke monitor/temp_basal.json 2>/dev/null >/dev/null && echo ed && openaps enact); else echo temp_basal.json less than 5m old; fi"'
...
openaps alias add refresh-profile '! bash -c "find settings/ -mmin -10 -size +5c | grep -q settings.json && echo Profile less than 10m old || (echo -n Settings refresh && openaps get-settings 2>/dev/null >/dev/null && echo ed)"'
