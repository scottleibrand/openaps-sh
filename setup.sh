...
openaps alias add gather '! bash -c "rm monitor/*; ( openaps get-bg | egrep \"reporting|Copied\" && echo -n R && openaps report invoke monitor/status.json 2>/dev/null >/dev/null && echo -n e && test $(cat monitor/status.json | json bolusing) == false && echo -n fr && (openaps monitor-pump || openaps monitor-pump) >/dev/null && echo -n esh && ( openaps ns-meal-carbs && echo ed ) || (echo; sleep 60; exit 1)) 2>/dev/null"' || die "Can't add gather"
